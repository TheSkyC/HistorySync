...
"""Bookmarks, bookmark tags, and per-URL annotations.

The legacy ``bookmarks.tags`` CSV column is still written (for backward
compatibility with old client versions) but every read goes through the
normalised ``bookmark_tags`` table.  ``add_bookmark`` and
``update_bookmark_tags`` keep the two stores in sync atomically.
"""
