...
class _BookmarksMixin:
    ...
    def get_bookmarks_page(
        ...
        # Tag filter — uses an EXISTS so it composes cleanly with the cursor
        # and other filters, without requiring a GROUP BY + HAVING.
        if page_filter.tag:
            clauses.append("EXISTS (SELECT 1 FROM bookmark_tags bt WHERE bt.bookmark_id = b.id AND bt.tag = ?)")
            params.append(page_filter.tag)

        # Annotation-presence filter ("Has Note" toggle and `has:note` token).
        if page_filter.has_annotation:
            clauses.append("EXISTS (SELECT 1 FROM annotations a WHERE a.url = b.url AND a.note != '')")

        # Date range — bounds are inclusive at day granularity to match the
        # previous Python-side filter (which used midnight / 23:59:59 endpoints).
        ...
        # Domain filter — substring match against the materialised host column,
        # mirroring the previous Python-side `domain_token in netloc` semantics.
        ...
        # Keyword filter — title / url / tag substring search.
        ...
        # Exclude tokens — matched against title || ' ' || url like the
        # previous Python implementation (NOT-LIKE composes additively).
        ...
        # Keyset cursor — strictly after (bookmarked_at, id) in (DESC, DESC).
