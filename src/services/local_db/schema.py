...
"""Schema creation, migrations and FTS integrity verification.

The mixin defined here is composed into :class:`LocalDatabase` from the
package ``__init__``.  It owns the *idempotent* DDL: creating tables on a
fresh database, applying incremental migrations to existing ones, and
auto-rebuilding the FTS index when corruption is detected at startup.
"""
