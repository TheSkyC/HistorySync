...
"""Database housekeeping: VACUUM, tombstone pruning, domain back-fill, etc.

These are long-running operations that release ``self._lock`` for the
expensive part of the work and rely on SQLite's own file-level locking to
serialise concurrent writers.  ``replace_database`` and ``export_without_fts``
share the same close/reopen pattern as ``vacuum_and_analyze`` so that
read-only callers (search overlay) keep working through the maintenance
window.
"""
