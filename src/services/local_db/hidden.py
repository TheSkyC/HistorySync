...
"""Soft-hide records and domains.

Two layers of hiding live here:

* ``hidden_records`` — URL-level soft-hide; the underlying row remains in
  ``history`` so it is still synced across devices, but the UI excludes it
  from normal views.
* ``hidden_domains`` — domain-level soft-hide; future syncs that bring in
  matching URLs are filtered out by the SQL fragments produced by
  :meth:`_HiddenMixin._hr_filter` and :meth:`_HiddenMixin._hd_filter`.

Both filter helpers are used by the stats and search-quick code paths via
``self.``, so they live on the same mixin even though they look like pure
helpers.
"""
