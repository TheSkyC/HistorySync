...
"""``history`` table CRUD, query builder, and the search_quick overlay path.

This is the largest mixin because the history table is the application's hot
path: every page in the UI calls :meth:`_HistoryMixin.get_records` and
:meth:`_HistoryMixin.get_filtered_count`, both of which delegate to the
shared :meth:`_HistoryMixin._build_query_parts`.

The module does *not* import the FTS query helpers directly — instead they
are accessed via the package namespace (``import src.services.local_db as
_pkg`` then ``_pkg._build_fts_query``) so that monkey-patches applied by
the test suite at the package level take effect inside these methods.

Backup-stats CRUD (``update_backup_stats``, ``get_all_backup_stats``,
``get_last_sync_time``) lives here too: it reads the same ``history`` write
path metadata so keeping them adjacent simplifies cross-referencing.
"""
