...
Devices are the cross-machine identity used by WebDAV sync to attribute
history rows to a particular installation.  This mixin exposes the public
helpers used by the device manager and the merge path; FTS-impacting writes
(merge, delete) call ``self._ensure_fts_triggers`` from the FTS mixin.
"""
