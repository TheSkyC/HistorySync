...
class MainWindow(QMainWindow):
    ...
    def _switch_page(self, index: int):
        ...
        if index == PAGE_HISTORY and self._page_history is None:
            ...
            self._page_history.bookmark_changed.connect(self._on_bookmark_changed_external)
    ...
    def _on_bookmark_changed_external(self):
        """Bookmark mutated by another page -- pull the change into bookmarks page.

        Wired to history page's bookmark_changed signal.  Calls the page's
        public `apply_external_bookmark_change()` which re-runs the active
        SQL filter (cheap with keyset pagination + materialised host column).
        """
        self._on_bookmark_changed()
        if self._page_bookmarks is not None:
            self._page_bookmarks.apply_external_bookmark_change()
