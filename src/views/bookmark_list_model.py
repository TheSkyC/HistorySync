...
class BookmarkListModel(QAbstractListModel):
    ...
    def refresh_loaded_url(self, url: str) -> bool:
        """Refresh a loaded row from DB, removing it if it no longer matches the active filter."""
        row = self.find_row_for_url(url)
        if row < 0:
            return False
        bm = self._db.get_bookmark_for_filter(url, self._filter)
        if bm is None:
            self.remove_url(url)
            return False
        ann = self._db.get_annotation(url)
        self.replace_record(bm, ann)
        return True

    def sync_external_change(self, action: str, url: str) -> bool:
        """Apply an external bookmark add/remove while preserving scroll state when possible."""
        if action == "removed":
            row = self.find_row_for_url(url)
            if row >= 0:
                self.remove_url(url)
    ...
    def _refresh_total_count(self) -> None:
        try:
            count = self._db.count_bookmarks(self._filter)
        except Exception:
            log.exception("count_bookmarks failed during external sync")
            return
