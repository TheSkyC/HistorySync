...
class LanguageManager(QObject):
    ...
    def __init__(self) -> None:
        ...
        # Identity function — returns the English key as-is (used for en_US / no translation)
    ...
    def get_best_match(self) -> str:
        ...
        supported_lower = {l.lower(): l for l in self.supported_languages}
    ...
    def setup_translation(self, lang_code: str | None = None) -> None:
        ...
        if lang_code == self.default_lang:
            # English: identity function (key == translation)
            self._translate = lambda s: s
        ...
        try:
            ...
            log.warning("Failed to load translation for '%s': %s — using English", lang_code, exc)
            self._translate = lambda s: s
            self.current_lang_code = self.default_lang
