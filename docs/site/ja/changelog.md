...
### Added
- desktop アプリにオンラインアップデート機能を内蔵し、プラットフォームとインストール形態に応じたパッケージ選択、リリースチャネルの切り替え、主要な更新 API が利用できない場合の GitHub Releases へのフォールバックに対応しました。
- 更新バナー、リリース詳細ダイアログ、通知頻度の設定、更新ポリシー・取得元の優先順位・チャネル選択のための専用設定セクションを追加しました。
- headless 環境で更新確認を行う `hsync update` を追加し、人間向けの出力、JSON 出力、一時的なチャネル上書きに対応しました。
- カスタムブラウザの実行モデルを統一し、手動追加したブラウザや詳細スキャンで見つかったブラウザも、同期・監視・UI 上で組み込みブラウザと同等に扱われるようになりました。
- ログビューアで自動 tail を開始し、ページが非表示またはウィンドウが最小化されたときは自動で一時停止するようになりました。

### Changed
- WebDAV 設定を整理し、より分かりやすくミスしにくい設定フローにしました。
- ブックマークページの描画を仮想化し、大規模データセットでのスケーラビリティを大幅に改善しました。

### Fixed
- ブラウザ監視状態が誤って `NEEDS_SYNC` へ切り替わることがなくなり、同期の新しさは抽出時に取得したデータベース mtime と比較されるようになりました。
- WebDAV 認証情報へのアクセスを実際の転送開始時まで遅延させ、重複アクセスを排除することで、不要な keyring アクセスや一部環境での繰り返しアンロック要求を減らしました。
- オプションの WebDAV 依存関係は転送開始時にのみ読み込まれるようになり、一部依存関係が欠けた環境での起動耐性が向上しました。
- カスタムパスで追加したブラウザは、起動時および設定保存直後に登録されるようになり、再起動なしで同期と状態監視に参加できるようになりました。
- `browser:` 検索フィルターは大文字小文字を区別しなくなり、ドメイン一致では SQL `LIKE` ワイルドカードを正しくエスケープして誤一致や過剰一致を防ぐようになりました。
- 無効化されたブラウザは状態表示から消えるのではなく、ダッシュボード上で明示的に `DISABLED` と表示されるようになりました。
- 履歴スクロールバブルは右端を優先し、画面端では左側へ自動的に退避するようになりました。
- ダッシュボードの詳細スキャン後処理で scroll area widget を二重解放する問題を修正しました。
- 危険性のあるカスタムブラウザ削除では、破壊的操作の前に確認を求めるようになりました。
- 統計ページの月ラベルはシステムロケールに依存せず、アプリの i18n 月名を一貫して使用するようになりました。
- トレイ動作の一貫性を改善し、全プラットフォームでシングルクリック開閉が動作し、ライトテーマでの同期スピナーも見やすくなりました。
- 中央配置ダイアログと StyledComboBox のポップアップは画面内に収まるよう制限され、マルチモニターや画面端での挙動が改善しました。
- ダッシュボードの統計カードはレスポンシブになり、ウィンドウサイズが変わっても安定した高さを保つようになりました。
- 更新確認が GitHub のスロットリングで UI をブロックしないようになりました。

### Build
- リリースのパッケージングと検証ワークフローを各プラットフォームで拡張・強化し、インストーラ検証、ARM64 インストーラフラグの修正、CI における依存関係インストールの堅牢化を含めました。

### Docs
- 1.4.0 に合わせてドキュメントサイトを更新し、オンラインアップデート、更新通知、CLI からの更新確認、改訂されたインストールとアップグレード手順を追加しました。
