...
1.4.0 以降、パッケージ版の desktop ビルドには **アプリ内アップデートシステム** も含まれます。

- **Windows インストーラ版** は、通常そのままプラットフォームのインストーラに処理を引き渡せます。
- **macOS `.dmg` 版** は、ディスクイメージをダウンロードして開くところまで実行できます。
- **ポータブル版、AppImage、アーカイブ版** は、適切な検証済みアーティファクトをダウンロードし、安全に置き換えられるようファイルを表示します。
- **`.deb` などのシステム管理インストール** は、引き続きパッケージマネージャまたは Releases ページ経由で更新します。

これは **設定 → 更新 → 更新を確認** から起動でき、新しいリリースが見つかったときは更新バナーからも詳細画面に進めます。

ターミナル派なら、次のコマンドも使えます。
...
!!! tip "ポータブルインストール"
    ポータブルインストールや `.portable` マーカーベースの構成では、設定とデータベースが実行ファイルの隣に保持されます。アップデートシステムもそのモデルを維持し、可能であればポータブル向けのリリースアーティファクトを選択します。
