...
### 如果 HistorySync 沒有自動辨識出我的瀏覽器怎麼辦？

可以。使用 **設定 → 瀏覽器 → 新增自訂路徑** 手動註冊瀏覽器。從 1.4.0 起，自訂瀏覽器與深度掃描發現的瀏覽器共享同一套執行模型，因此儲存設定後無需重新啟動，也能立即參與同步、狀態監控與啟用/停用控制。
...
HistorySync 會偵測你目前是安裝版、可攜版、AppImage、`.dmg` 還是系統套件管理安裝。

- 安裝版通常可以直接把升級流程交給平台安裝程式。
- 可攜版、AppImage 與封存版會下載正確檔案，並在檔案總管中定位，方便你手動安全替換。
- 透過套件管理器安裝的版本，例如 `.deb`，會繼續建議使用系統套件管理器升級，而不是在應用程式內直接替換檔案。

你可以在 **設定 → 更新** 中手動檢查，也可以在終端機執行 `hsync update`。
...
它會回傳適合自動化處理的結構化中繼資料。也可以透過 `--channel beta` 或 `--channel nightly` 臨時指定檢查通道。
