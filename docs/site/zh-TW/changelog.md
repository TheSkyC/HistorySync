...
### 新增
- 內建線上更新系統，支援依平台與安裝形態選擇合適的安裝包、切換發布通道，並在主要更新來源不可用時回退到 GitHub 發布資訊。
- 更新橫幅、版本詳情對話框、提醒頻率設定，以及獨立的「更新」設定區，可配置更新策略、下載來源偏好與通道。
- 新增 `hsync update`，可在無介面環境中檢查更新，支援人類可讀輸出、JSON 輸出與臨時通道覆寫。
- 統一的自訂瀏覽器執行模型，使手動新增的瀏覽器與深度掃描發現的瀏覽器在同步、監控與 UI 中都像內建瀏覽器一樣運作。
- 日誌檢視器開啟時自動開始 tail，頁面隱藏或視窗最小化時自動暫停。

### 變更
- WebDAV 設定介面重新梳理，配置流程更簡潔，也更不容易出錯。
- 書籤頁渲染改為虛擬化，在大型資料集下有更好的可擴充性。

### 修正
- 瀏覽器監視器不再誤報 `NEEDS_SYNC`；同步新鮮度現在改為依提取時記錄的資料庫 mtime 快照判斷。
- WebDAV 憑證存取現在延遲到真正傳輸時才發生，並會去重，減少不必要的 keyring 存取與重複解鎖提示。
- 可選 WebDAV 依賴現在只在開始傳輸時載入，提升缺少相關依賴環境下的啟動穩健性。
- 透過自訂路徑新增的瀏覽器現在會在啟動時與儲存設定後立即註冊，無需重新啟動即可參與同步與狀態監控。
- `browser:` 搜尋過濾現已大小寫不敏感；網域比對也會正確跳脫 SQL `LIKE` 萬用字元，避免誤比對與誤傷資料。
- 已停用的瀏覽器現在會在儀表板中明確顯示為 `DISABLED`，而不是直接從狀態檢視中消失。
- 歷史頁捲動氣泡現在優先貼右側，接近螢幕邊緣時會自動回退到左側。
- 修正儀表板深度掃描清理階段對 scroll area widget 的重複釋放問題。
- 移除高風險自訂瀏覽器前現在會要求確認。
- 統計頁月份標籤不再依賴系統區域設定，統一改用應用程式自身的 i18n 月份名稱。
- 系統匣行為更加一致：所有平台都支援單擊開啟主視窗，淺色主題下的同步旋轉指示器也更清楚。
- 置中的對話框與 StyledComboBox 彈層現在會限制在螢幕範圍內，多顯示器與貼邊場景表現更穩定。
- 儀表板統計卡片現在具備回應式版面，並能在不同視窗尺寸下保持穩定高度。
- 更新檢查現在不會因 GitHub 節流而阻塞介面流程。

### 建置
- 擴展並強化了跨平台發布打包與驗證流程，包括安裝包驗證、ARM64 安裝程式參數修正，以及 CI 中依賴安裝流程的強化。

### 文件
- 文件站點已針對 1.4.0 更新，補充了線上更新、更新提醒、CLI 更新檢查，以及新的安裝與升級說明。
