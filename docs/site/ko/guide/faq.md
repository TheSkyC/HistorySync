...
### HistorySync가 자동으로 감지하지 못하는 브라우저도 추가할 수 있나요?

네. **설정 → 브라우저 → 사용자 지정 경로 추가**를 사용해 브라우저를 수동으로 등록할 수 있습니다. 1.4.0부터 사용자 지정 브라우저는 기본 제공 브라우저와 심층 스캔으로 찾은 브라우저와 동일한 런타임 모델을 사용하므로, 설정 저장 후 재시작 없이도 동기화, 상태 모니터링, 활성화/비활성화 제어에 참여합니다.
...
### 새 업데이트 시스템은 설치 형태에 따라 어떻게 동작하나요?

HistorySync는 현재 설치가 설치 프로그램 버전인지, 포터블 버전인지, AppImage인지, `.dmg`인지, 시스템 관리 패키지인지 감지합니다.

- 설치 프로그램 형태의 빌드는 일반적으로 플랫폼 설치 프로그램에 바로 절차를 넘길 수 있습니다.
- 포터블, AppImage, 아카이브 설치는 올바른 아티팩트를 다운로드하고 안전하게 수동 교체할 수 있도록 파일 위치를 보여줍니다.
- `.deb` 같은 패키지 관리자 설치는 파일을 직접 덮어쓰지 않고 계속 패키지 관리자를 사용합니다.

언제든 **설정 → 업데이트**에서 수동으로 확인할 수 있고, 터미널에서는 `hsync update`를 실행할 수 있습니다.
...
자동화에 적합한 구조화된 메타데이터가 반환됩니다. 호출마다 `--channel beta` 또는 `--channel nightly`로 채널을 덮어쓸 수도 있습니다.
...
**공개 이슈를 열지 마세요.** 설명, 재현 단계, 잠재적 영향을 포함하여 **security@historysync.app**으로 직접 이메일을 보내주세요. 72시간 이내에 응답을 받으실 수 있습니다.
