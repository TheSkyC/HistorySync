...
1.4.0부터 패키지된 desktop 빌드에는 **앱 내 업데이트 시스템**도 포함됩니다.

- **Windows 설치 프로그램 빌드**는 일반적으로 플랫폼 설치 프로그램에 바로 업그레이드 절차를 넘길 수 있습니다.
- **macOS `.dmg` 빌드**는 디스크 이미지를 다운로드하고 열어줄 수 있습니다.
- **포터블, AppImage, 아카이브 설치**는 적절한 검증된 아티팩트를 다운로드한 뒤 안전하게 교체할 수 있도록 위치를 보여줍니다.
- **`.deb` 같은 시스템 관리형 설치**는 계속해서 패키지 관리자나 Releases 페이지를 통해 업데이트합니다.

이 기능은 **설정 → 업데이트 → 업데이트 확인**에서 실행할 수 있으며, 새 릴리스가 감지되면 업데이트 배너에서도 상세 화면으로 이동할 수 있습니다.

터미널을 선호한다면 다음 명령도 사용할 수 있습니다.
...
!!! tip "포터블 설치"
    포터블 설치와 `.portable` 마커 기반 구성은 설정과 데이터베이스를 실행 파일 옆에 유지합니다. 업데이트 시스템도 이 모델을 보존하며, 가능하면 포터블 형식의 릴리스 아티팩트를 선택합니다.
