...
### Можно ли добавить браузеры, которые HistorySync не обнаруживает автоматически?

Да. Используйте **Настройки → Браузеры → Добавить пользовательский путь**, чтобы зарегистрировать браузер вручную. Начиная с версии 1.4.0, пользовательские браузеры используют ту же модель выполнения, что и встроенные браузеры и браузеры, найденные глубокой проверкой, поэтому они участвуют в синхронизации, мониторинге состояния и управлении включением/отключением без перезапуска приложения после сохранения настроек.
...
### Как новая система обновлений ведёт себя для разных типов установки?

HistorySync определяет, используете ли вы установочную сборку, портативную сборку, AppImage, `.dmg` или пакет, управляемый системой.

- Сборки в формате установщика обычно могут сразу передать управление установщику платформы.
- Портативные, AppImage и архивные установки скачивают правильный артефакт и показывают его для безопасной ручной замены.
- Установки через менеджер пакетов, такие как `.deb`, продолжают использовать менеджер пакетов вместо замены файлов на месте.

Вы всегда можете выполнить ручную проверку в **Настройки → Обновления**, либо запустить `hsync update` из терминала.
...
### Можно ли проверять релизы из скриптов?
...
Команда вернёт структурированные метаданные, подходящие для автоматизации. Вы также можете переопределить канал для конкретного вызова через `--channel beta` или `--channel nightly`.
